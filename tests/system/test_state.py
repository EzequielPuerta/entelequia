...
def test_create_system_stopped(attrs_mock_system: AttrsMockSystem):
    ...
    assert state.start_up() == None
    with pytest_raises(AssertionError) as exception:
        state.shut_down()
    assert str(exception.value) == f"{attrs_mock_system.name} is already stopped."
...
def test_create_system_started(attrs_mock_system: AttrsMockSystem):
    ...
    with pytest_raises(AssertionError) as exception:
        state.start_up()
    assert str(exception.value) == f"{attrs_mock_system.name} is already started."
    assert state.shut_down() == None
